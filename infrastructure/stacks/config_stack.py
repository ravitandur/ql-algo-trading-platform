...
class ConfigurationStack(Stack):
    ...
    def _create_parameter_store_entries(self) -> Dict[str, ssm.StringParameter]:
        """
        ...
        for param_name, param_value in self.env_config.custom_parameters.items():
    ...
    def _get_environment_parameters(self) -> Dict[str, str]:
        ...
        return {
            ...
            "vpc/enable-nat-gateway": str(self.env_config.networking.enable_nat_gateway).lower(),
            
            # Resource configuration
            "lambda/memory-size": str(self.env_config.resources.lambda_memory_size),
            ...
            "monitoring/log-retention-days": str(self.env_config.monitoring.log_retention_days),
            "monitoring/enable-xray": str(self.env_config.monitoring.enable_xray_tracing).lower(),
            "monitoring/enable-detailed": str(self.env_config.monitoring.enable_detailed_monitoring).lower(),
            
            # Security configuration
            "security/enable-waf": str(self.env_config.security.enable_waf).lower(),
            "security/enable-encryption": str(self.env_config.security.enable_encryption_at_rest).lower(),
            
            # Compliance configuration
            "compliance/data-residency": self.env_config.compliance.data_residency_region,
            "compliance/backup-retention": str(self.env_config.compliance.backup_retention_days),
            
            # Cost optimization
            "cost/enable-spot": str(self.env_config.cost_optimization.enable_spot_instances).lower(),
            "cost/enable-lifecycle": str(self.env_config.cost_optimization.enable_lifecycle_policies).lower(),
        }
    ...
    def _get_secure_parameters(self) -> Dict[str, str]:
        ...
        return {
            ...
            "notifications/email-endpoint": self.env_config.monitoring.alarm_notification_email or "admin@example.com",
            
            # Encryption keys
            "encryption/data-key": "placeholder-will-be-generated",
        }

    def _create_secrets_manager_entries(self) -> Dict[str, secretsmanager.Secret]:
        """
        ...
        secrets["external_api_keys"] = secretsmanager.Secret(
            ...
            encryption_key=self.config_kms_key,
            secret_object_value={
                "trading_api_key": secretsmanager.SecretValue.unsafe_plain_text("placeholder-update-after-deployment"),
                "market_data_key": secretsmanager.SecretValue.unsafe_plain_text("placeholder-update-after-deployment"),
                "notification_webhook": secretsmanager.SecretValue.unsafe_plain_text("placeholder-update-after-deployment"),
            },
        )
        ...
        secrets["application_secrets"] = secretsmanager.Secret(
            ...
            encryption_key=self.config_kms_key,
            secret_object_value={
                "jwt_secret": secretsmanager.SecretValue.unsafe_plain_text("placeholder-will-be-generated"),
                "encryption_key": secretsmanager.SecretValue.unsafe_plain_text("placeholder-will-be-generated"),
                "session_secret": secretsmanager.SecretValue.unsafe_plain_text("placeholder-will-be-generated"),
            },
        )
    ...
    def get_parameter_arn(self, parameter_name: str) -> str:
        ...
        Returns:
            str: Parameter ARN
        """
        full_name = f"{self.env_config.get_parameter_store_prefix()}/{parameter_name}"
    ...
    def grant_parameter_access(
        ...
        for path in parameter_paths:
            full_path = f"{self.env_config.get_parameter_store_prefix()}/{path}"
            parameter_arns.append(f"arn:aws:ssm:{self.region}:{self.account}:parameter{full_path}/*")
