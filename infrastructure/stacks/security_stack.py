...
class SecurityStack(Stack):
    ...
    def _create_security_groups(self) -> Dict[str, ec2.SecurityGroup]:
        ...
        # Application Services Security Group
        security_groups["app"] = self._create_app_security_group(security_groups["alb"])
        
        # Database Services Security Group
        security_groups["db"] = self._create_database_security_group(security_groups["app"])
        
        # Cache Services Security Group
        security_groups["cache"] = self._create_cache_security_group(security_groups["app"])
        
        # Management/Bastion Security Group
        security_groups["management"] = self._create_management_security_group()
    ...
    def _create_app_security_group(self, alb_sg: ec2.SecurityGroup) -> ec2.SecurityGroup:
    ...
    def _create_database_security_group(self, app_sg: ec2.SecurityGroup) -> ec2.SecurityGroup:
    ...
    def _create_cache_security_group(self, app_sg: ec2.SecurityGroup) -> ec2.SecurityGroup:
    ...
    def get_security_group(self, name: str) -> ec2.SecurityGroup:
        ...
        if name not in self.security_groups:
            raise KeyError(f"Security group '{name}' not found. Available: {list(self.security_groups.keys())}")
    ...
    def add_ingress_rule_to_group(
        ...
        sg.add_ingress_rule(peer=peer, connection=connection, description=description)