...
class OptionsStrategyIAMConstruct(Construct):
    ...
    def _get_ecs_execution_policy_statements(self) -> List[iam.PolicyStatement]:
        """
    ...
    def create_custom_role(
        ...
        role = iam.Role(
            ...
            description=description or f"Custom role for {role_name} in {self.env_name} environment",
        )
        ...
        if managed_policies:
            for policy_arn in managed_policies:
                role.add_managed_policy(
                    iam.ManagedPolicy.from_managed_policy_arn(self, f"{role_id}ManagedPolicy", policy_arn)
                )
